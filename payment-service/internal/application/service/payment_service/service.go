...
type PaymentRepository interface {
	...
	UpdateStatuses(ctx context.Context, operationID uuid.UUID, paymentStatus, operationStatus string) error
}
...
func (s *service) Process(ctx context.Context, w http.ResponseWriter, paymentRequest dto.PaymentRequest) (status string, err error) {
	paymentRequest.OperationStatus = "processing"
	...
	err = s.paymentRepository.UpdateStatuses(ctx, paymentRequest.OperationID, paymenStatus, "completed")
	if err != nil {
...
func (s *service) paymentRequest(ctx context.Context, w http.ResponseWriter, paymentRequest dto.PaymentRequest) (status string, err error) {
	bankAPIURL := "http://bank-api/payment"
	jsonData, err := json.MarshalIndent(paymentRequest, "", "\t")
	if err != nil {
		return "error", fmt.Errorf("client: could not create request: %s", err)
	}

	request, err := http.NewRequest(http.MethodPost, bankAPIURL, bytes.NewBuffer(jsonData))
	if err != nil {
		return "error", fmt.Errorf("client: could not create request: %s", err)
	}
	request.Close = true
